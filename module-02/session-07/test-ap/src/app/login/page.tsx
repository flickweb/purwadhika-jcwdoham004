...
export default function Login() {
  ...
  const initialValues: ILogin /* tipe ILogin */ = { email: "", password: "" }; 
  ...
  const login = async (values: ILogin) =>{
    try{
        ...
        // await fetch(
            `https://wealthyworm-us.backendless.app/api/data/user?where=email%3D'${values.email}'%20and%20password%3D'${values.password}`); 
            // harusnya ga boleh buat kirim email and password, no need to hafal right now
            
            //method get gabisa di backendless kyk below.
            //by default kalo gaada method/body, dia jadi get
        ...
        */
        ...
        //ini karna pake backendless, harus ada ginian like above
  ...
  return (
    <div className="flex flex-col items-center justify-center mt-[80px] gap-6">
      ...
      <Formik<ILogin> //ini ngasih tau frmik bahwa ada ILogin sebagai generic type, biar formik tau ada value email and password
        initialValues={initialValues}// ngasih tau emailnya kosong and passwordnya kosong
        validationSchema={LoginSchema} // submit gaakan jalan kalo gagal validation
        onSubmit={(values) => login/*function login*/(values)} //apa yang terjadi if submitted. in this case bakal nglakuin function, no need useState or ref krna di handle formik
      >
        {(formik: FormikProps<ILogin> /* nerima property which is formik, typenya formikprops, lalu kita kasih ILogin (artinya valuenya 2 aja, email and password)*/) => {
          return (
            <Form className="flex flex-col gap-5">
              <div className="flex flex-col gap-2">
                ...
                <input
                  ...
                  type="text"
                  name="email"
                  value={formik.values.email} //formik punya value, and value nya email karena dr ILogin ada email and initialnya skrg kosong. 
